from bible_cli.constants import DEFAULT_TRANSLATION, QUERY_PATTERN
from bible_cli.database import Book, Translation, Verse, get_session
from bible_cli.schemas import QueryData
...
class QueryProcessor:
    ...
    def _parse_query(self, raw_query) -> QueryData:
        ...
        query_string, raw_translation = self._extract_query_string_and_translation(
            raw_query
        )
    ...
    @staticmethod
    def _build_query_data(query_string, translation):
        ...
        query_data = QueryData(
            query_string, translation, book_name, chapter, verse_start, verse_end
        )

        return query_data

    @staticmethod
    def _get_translation(translation_str: str) -> Translation:
        try:
    ...
    @staticmethod
    def _is_valid_translation(translation_str: str) -> bool:
        return translation_str.upper() in Translation.__members__

    @staticmethod
    def _is_valid_query_range(split_query: list[str]):
        return len(split_query) > 1

    def _extract_query_string_and_translation(
